...
You may also provide all these options with M<new()> directly.
...
If you use the M<XML::Compile::Cache> object, which extends ::Schema, then
...
Many schema's are broken.  M<XML::Compile> is not a good tool to figure-out
